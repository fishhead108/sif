...
in
{
  ...
  config = mkIf (cfg.enable) {
    ...
    systemd = {
      ...
      # hybrid-sleep broken on lenovo: https://community.frame.work/t/issues-with-sleep-states-on-linux/7363
      # lenovo: cat /sys/power/mem_sleep -> [s2idle] deep
      # Change suspendstate to deep for lenovo
      sleep.extraConfig = ''
    ...
    services = {
      # Hibernate on low battery. from: https://wiki.archlinux.org/title/dell#Hibernate_on_low_battery_level
      udev.extraRules = ''
      ...
      acpid = {
        # NixOS source: https://github.com/NixOS/nixpkgs/blob/nixos-22.05/nixos/modules/services/hardware/acpid.nix
      ...
      tlp = {
        # Nix source: https://github.com/NixOS/nixpkgs/blob/nixos-22.05/nixos/modules/services/hardware/tlp.nix
        ...
        settings = {
          ...
          "USB_AUTOSUSPEND" = 0;
        } // (if config.jd.lenovo.enable == true then {
